...
const struct m_sub_options vd_lavc_conf = {
    .opts =
        (const m_option_t[]){
            ...
            {"vd-lavc-film-grain",
             OPT_CHOICE(film_grain, {"auto", -1}, {"cpu", 0}, {"gpu", 1})},
            ...
            {"vd-lavc-software-fallback",
             OPT_CHOICE(software_fallback, {"no", INT_MAX}, {"yes", 1}),
             M_RANGE(1, INT_MAX)},
            {"vd-lavc-o", OPT_KEYVALUELIST(avopts)},
            {"vd-lavc-dr", OPT_CHOICE(dr, {"auto", -1}, {"no", 0}, {"yes", 1})},
            {"hwdec", OPT_STRINGLIST(hwdec_api), .help = hwdec_opt_help,
            ...
            {"hwdec-extra-frames", OPT_INT(hwdec_extra_frames),
             M_RANGE(0, 256)},
            {0}},
    .size = sizeof(struct vd_lavc_params),
    .defaults =
        &(const struct vd_lavc_params){
            .film_grain = -1 /*auto*/,
            ...
            // Maximum number of surfaces the player wants to buffer. This
            // number might require adjustment depending on whatever the player
            // does; for example, if vo_gpu increases the number of reference
            // surfaces for interpolation, this value has to be increased too.
...
static void add_hwdec_item(struct hwdec_info **infos, int *num_infos,
  ...
  snprintf(info.name, sizeof(info.name), "%s-%s", info.codec->name,
           info.method_name);
...
static AVBufferRef *hwdec_create_dev(struct mp_filter *vd,
                                     struct hwdec_info *hwdec, bool autoprobe) {
  vd_ffmpeg_ctx *ctx = vd->priv;
...
static void select_and_set_hwdec(struct mp_filter *vd) {
  ...
  for (int i = 0; hwdec_api[i]; i++) {
    ...
    bool hwdec_auto_all = bstr_equals0(opt, "auto") || bstr_equals0(opt, "");
    bool hwdec_auto_safe = bstr_equals0(opt, "auto-safe") ||
                           bstr_equals0(opt, "auto-copy-safe") ||
                           bstr_equals0(opt, "yes");
    bool hwdec_auto_copy =
        bstr_equals0(opt, "auto-copy") || bstr_equals0(opt, "auto-copy-safe");
    ...
    if (!hwdec_requested) {
      ...
      for (int n = 0; n < num_hwdecs; n++) {
         ...
         */
        MP_TARRAY_APPEND(ctx, ctx->attempted_hwdecs, ctx->num_attempted_hwdecs,
...
static void force_fallback(struct mp_filter *vd) {
  ...
  mp_msg(vd->log, lev,
         "Attempting next decoding method after failure of %.*s.\n",
...
static void init_avctx(struct mp_filter *vd) {
  ...
  if (!ctx->use_hwdec && ctx->vo && lavc_param->dr) {
    ...
    AV_NOWARN_DEPRECATED({ avctx->thread_safe_callbacks = 1; });
#endif
  }
  ...
  case 0: /*CPU*/
    ...
    break;
  case 1: /*GPU*/
    if (!ctx->vo || (ctx->vo && !(ctx->vo->driver->caps & VO_CAP_FILM_GRAIN))) {
      MP_MSG(
          vd, ctx->vo ? MSGL_WARN : MSGL_V,
...
static int init_generic_hwaccel(struct mp_filter *vd,
  ...
  if (avcodec_get_hw_frames_parameters(ctx->avctx, ctx->hwdec_dev, hw_fmt,
                                       &new_frames_ctx) < 0) {
