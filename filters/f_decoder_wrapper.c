 ...
 */
 ...
 static void decf_reset(struct mp_filter *f)
 {
     ...
     p->last_output_pts = MP_NOPTS_VALUE;
 ...
 static void read_frame(struct priv *p)
 {
     ...
     if (atomic_load(&enable_cartrack) && frame.type == MP_FRAME_VIDEO) {
         double pts = mp_frame_get_pts(frame);    
         if (pts != MP_NOPTS_VALUE) {
             if (p->last_output_pts != MP_NOPTS_VALUE) {
                 double delta = pts - p->last_output_pts + 0.0001;
                 if (delta < 1.0 / 30.0) {
                     MP_ERR(p, "Frame too close to previous frame: %f\n", delta);
                     mp_frame_unref(&frame);
                     mp_filter_internal_mark_progress(p->decf);
                     return;
                 }
             }
             p->last_output_pts = pts;
         }
         ...
         if (can_track && mpv_cartrack_process != NULL) {
